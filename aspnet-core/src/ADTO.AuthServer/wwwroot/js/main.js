(function ($) {
    ...
    adto.event.on('adto.notifications.received', function (userNotification) {
        adto.notifications.showUiNotifyForUserNotification(userNotification);

        //Desktop notification
        Push.create("AuthServer", {
            body: userNotification.notification.data.message,
            icon: adto.appPath + 'img/logo.png',
            timeout: 6000,
            onClick: function () {
                window.focus();
                this.close();
    ...
    function initAdvSearch() {
        $('.adto-advanced-search').each((i, obj) => {
            var $advSearch = $(obj);
