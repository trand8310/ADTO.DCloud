...
public class DCloudAsyncJwtSecurityTokenHandler : IAsyncSecurityTokenValidator
{
    private readonly JwtSecurityTokenHandler _tokenHandler;

    public DCloudAsyncJwtSecurityTokenHandler()
    {
    ...
    private static void ValidateUserDelegation(ClaimsPrincipal principal)
    {
        ...
        var impersonatorTenant = principal.Claims.FirstOrDefault(c => c.Type == ADTOSharpClaimTypes.ImpersonatorTenantId);
        var user = principal.Claims.FirstOrDefault(c => c.Type == ADTOSharpClaimTypes.UserId);
        var impersonatorUser = principal.Claims.FirstOrDefault(c => c.Type == ADTOSharpClaimTypes.ImpersonatorUserId);
        ...
        var impersonatorTenantId = impersonatorTenant == null ? null :
            impersonatorTenant.Value.IsNullOrEmpty() ? (Guid?)null : Guid.Parse(impersonatorTenant.Value);
        var sourceUserId = Guid.Parse(user.Value);
        var impersonatorUserId = Guid.Parse(impersonatorUser.Value);
