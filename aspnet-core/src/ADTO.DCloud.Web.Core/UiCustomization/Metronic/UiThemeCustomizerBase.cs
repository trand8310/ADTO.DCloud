...
namespace ADTO.DCloud.Web.UiCustomization.Metronic
{
    public class UiThemeCustomizerBase
    {
        ...
        protected async Task<string> GetSettingValueForTenantAsync(string settingName, Guid tenantId)
        {
            return await SettingManager.GetSettingValueForTenantAsync(ThemeName + "." + settingName, tenantId);
        }

        protected async Task<T> GetSettingValueForTenantAsync<T>(string settingName, Guid tenantId) where T : struct
        {
            return (await SettingManager.GetSettingValueForTenantAsync(ThemeName + "." + settingName, tenantId))
                .To<T>();
        }
        ...
        protected async Task ChangeSettingForTenantAsync(Guid tenantId, string name, string value)
        {
