...
using ADTO.DCloud.UiCustomization.Dto;
using System;
...
namespace ADTO.DCloud.Web.UiCustomization.Metronic
{
    public class ThemeDefaultUiCustomizer : UiThemeCustomizerBase, IUiCustomizer
    {
        ...
        public async Task UpdateTenantUiManagementSettingsAsync(Guid tenantId, ThemeSettingsDto settings,
        ...
        public async Task<ThemeSettingsDto> GetTenantUiCustomizationSettings(Guid tenantId)
        {
        ...
        private void ValidateSettings(ThemeSettingsDto settings)
        {
            if (settings.Menu.AllowAsideMinimizing && !settings.Menu.HoverableAside)
            {
                throw new UserFriendlyException(
                    _localizationManager.GetSource(DCloudConsts.LocalizationSourceName)
