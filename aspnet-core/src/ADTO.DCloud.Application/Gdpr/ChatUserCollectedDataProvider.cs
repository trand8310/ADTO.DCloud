...
using ADTO.DCloud.Chat.Exporting;
...
namespace ADTO.DCloud.Gdpr
{
    public class ChatUserCollectedDataProvider : IUserCollectedDataProvider, ITransientDependency
    {
        ...
        public async Task<List<FileDto>> GetFiles(UserIdentifier user)
        {
            ...
            Dictionary<Guid, string> relatedTenancyNames;
        ...
        private Dictionary<UserIdentifier, string> GetFriendUsernames(List<UserIdentifier> users)
        {
            ...
            using (_unitOfWorkManager.Current.DisableFilter(ADTOSharpDataFilters.SoftDelete))
            {
        ...
        private async Task<Dictionary<UserIdentifier, List<ChatMessageExportDto>>> GetUserChatMessages(Guid? tenantId, Guid userId)
        {
