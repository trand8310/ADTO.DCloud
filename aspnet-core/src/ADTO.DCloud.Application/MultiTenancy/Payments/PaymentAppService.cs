...
{
    [HiddenApi]
    public class PaymentAppService : DCloudAppServiceBase, IPaymentAppService
    {
        ...
        [ADTOSharpAuthorize(PermissionNames.Pages_Administration_Tenant_SubscriptionManagement)]
        ...
        public async Task<long> CreatePayment(CreatePaymentDto input)
        {
            ...
            {
                throw new ApplicationException("A payment only can be created for a tenant. TenantId is not set in the IADTOSharpSession!");
            }
        ...
        public async Task<PagedResultDto<SubscriptionPaymentListDto>> GetPaymentHistory(GetPaymentHistoryInput input)
        {
            var query = _subscriptionPaymentRepository.GetAll()
                ...
                .Where(sp => sp.TenantId == ADTOSharpSession.GetTenantId())
        ...
        public async Task SwitchBetweenFreeEditions(Guid upgradeEditionId)
        {
        ...
        public async Task UpgradeSubscriptionCostsLessThenMinAmount(Guid editionId)
        {
