...
/// 事件发布订阅服务的响应服务
/// </summary>
[ADTOSharpAuthorize(PermissionNames.Pages_Administration_Webhook_ListSendAttempts)]
public class WebhookSendAttemptAppService : DCloudAppServiceBase, IWebhookAttemptAppService
{
    #region Fields
    ...
    [ADTOSharpAuthorize(PermissionNames.Pages_Administration_Webhook_ResendWebhook)]
    public async Task Resend(string sendAttemptId)
    {
        var webhookSendAttempt = await _webhookSendAttemptStore.GetAsync(ADTOSharpSession.TenantId, Guid.Parse(sendAttemptId));
