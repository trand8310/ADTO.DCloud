...

{
    public partial class CommonHelper
    {
        ...
        public static string EnsureSubscriberEmailOrThrow(string email)
        {
            ...
            {
                throw new ADTOSharpException("邮件地址无效");
            }
        ...
        public static void SetProperty(object instance, string propertyName, object value)
        {
            ...
            var pi = instanceType.GetProperty(propertyName)
                     ?? throw new ADTOSharpException($"No property '{propertyName}' found on the instance of type '{instanceType}'.");

            if (!pi.CanWrite)
                throw new ADTOSharpException($"The property '{propertyName}' on the instance of type '{instanceType}' does not have a setter.");
        ...
        public static string GuidToBase36()
        {
        ...
        public static string ReplaceHtml(string s, ReplaceOptions replaceOptions)
        {
            ...
            {
                dictionary.Add(@"<script[\s\S]+?</script *>", "");
                dictionary.Add(@"<style[\s\S]+?</style *>", "");
                dictionary.Add(@"<iframe[\s\S]+?</iframe *>", "");
                dictionary.Add(@"<frameset[\s\S]+?</frameset *>", "");
                dictionary.Add(@" href *= *[\s\S]*?script *:", "");
                dictionary.Add(@"<!--[\s\S]+?-->", "");
            }
            ...
            {
                dictionary.Add(Environment.NewLine, "");
                dictionary.Add("  ", " ");
                if ((replaceOptions | ReplaceOptions.OutputAsMultiline) == replaceOptions)
                {
                    dictionary.Add(@"\<br[^\>]*\>", Environment.NewLine);
                    dictionary.Add(@"\</p[^\>]*\>", Environment.NewLine);
                }
                dictionary.Add("<[^>]*>", "");
            }
            foreach (KeyValuePair<string, string> pair in dictionary)
            {
                s = Regex.Replace(s, pair.Key, pair.Value, RegexOptions.IgnoreCase);
            }
