...
/// </summary>
public class UserEmailer : ADTOSharpServiceBase, IUserEmailer, ITransientDependency
{
    ...
    public async Task SendPasswordResetLinkAsync(User user, string link = null)
    {
        ...
        if (!link.IsNullOrEmpty())
        {
            ...
            var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours).ToString(DCloudConsts.DateTimeOffsetFormat));
    ...
    public async Task TryToSendSubscriptionExpireEmail(Guid tenantId, DateTime utcNow)
    {
        try
        {
    ...
    public async Task TryToSendSubscriptionAssignedToAnotherEmail(Guid tenantId, DateTime utcNow,
        Guid expiringEditionId)
    {
        try
        {
            using (_unitOfWorkManager.Begin())
    ...
    public async Task TryToSendSubscriptionExpiringSoonEmail(Guid tenantId, DateTime dateToCheckRemainingDayCount)
    {
        try
        {
            await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
    ...
    public void TryToSendPaymentNotCompletedEmail(Guid tenantId, string urlToPayment)
    {
        try
        {
    ...
    private string GetTenancyNameOrNull(Guid? tenantId)
    {
        if (tenantId == null)
        {
            return null;
    ...
    private StringBuilder GetTitleAndSubTitle(Guid? tenantId, string title, string subTitle)
    {
