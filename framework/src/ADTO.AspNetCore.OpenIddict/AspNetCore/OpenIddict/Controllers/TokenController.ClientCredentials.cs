...
public partial class TokenController<TTenant, TRole, TUser>
{
    ...
    {
        var ApplicationManager = IocManager.Instance.Resolve<IOpenIddictApplicationManager>();
        // Note: the client credentials are automatically validated by OpenIddict:
        // if client_id or client_secret are invalid, this action won't be invoked.
        var application = await ApplicationManager.FindByClientIdAsync(request.ClientId);
        if (application == null)
        {
            throw new InvalidOperationException("TheApplicationDetailsCannotBeFound");
        }
