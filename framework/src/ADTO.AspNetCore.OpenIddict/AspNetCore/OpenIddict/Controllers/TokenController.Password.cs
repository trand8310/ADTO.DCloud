...
public partial class TokenController<TTenant, TRole, TUser>
{
    protected virtual async Task<IActionResult> HandlePasswordAsync(OpenIddictRequest request)
    {
        ...
        var userManager = IocManager.Instance.Resolve<ADTOSharpUserManager<TRole, TUser>>();
        var signInManager = IocManager.Instance.Resolve<ADTOSharpSignInManager<TTenant, TRole, TUser>>();
    ...
    {
        var signInManager = IocManager.Instance.Resolve<ADTOSharpSignInManager<TTenant, TRole, TUser>>();
        var openIddictClaimsPrincipalManager = IocManager.Instance.Resolve<ADTOOpenIddictClaimsPrincipalManager>();
        ...
        // different from ADTO.io
        // ----------------------------------------------------------------------------------
        principal.SetClaim(OpenIddictConstants.Claims.Subject, user.Id.ToString());
        if (user.TenantId.HasValue)
        {
            principal.SetClaim(ADTOSharpClaimTypes.TenantId, user.TenantId?.ToString());
        }
        // ----------------------------------------------------------------------------------
