...
namespace ADTOSharp.Notifications
{
    ...
    public class UserNotificationManager : IUserNotificationManager, ISingletonDependency
    {
        ...
        public async Task<List<UserNotification>> GetUserNotificationsAsync(UserIdentifier user, UserNotificationState? state = null, int pageNumber = 0, int pageSize = int.MaxValue, DateTime? startDate = null, DateTime? endDate = null)
        {
            var userNotifications = await _store.GetUserNotificationsWithNotificationsAsync(user, state, pageNumber, pageSize, startDate, endDate);
            return userNotifications
                .Select(un => un.ToUserNotification())
                .ToList();
        }

        public List<UserNotification> GetUserNotifications(UserIdentifier user, UserNotificationState? state = null, int pageNumber = 0, int pageSize = int.MaxValue, DateTime? startDate = null, DateTime? endDate = null)
        {
            var userNotifications = _store.GetUserNotificationsWithNotifications(user, state, pageNumber, pageSize, startDate, endDate);
            return userNotifications
                .Select(un => un.ToUserNotification())
                .ToList();
        }
