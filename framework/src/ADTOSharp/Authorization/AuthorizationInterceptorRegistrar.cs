...
namespace ADTOSharp.Authorization
{
    ...
    internal static class AuthorizationInterceptorRegistrar
    {
        ...
        private static void Kernel_ComponentRegistered(string key, IHandler handler)
        {
            ...
            {
                handler.ComponentModel.Interceptors.Add(new InterceptorReference(typeof(ADTOSharpAsyncDeterminationInterceptor<AuthorizationInterceptor>))); 
            }
        }

        private static bool ShouldIntercept(Type type)
        {
            if (SelfOrMethodsDefinesAttribute<ADTOSharpAuthorizeAttribute>(type))
            {
                return true;
            }
