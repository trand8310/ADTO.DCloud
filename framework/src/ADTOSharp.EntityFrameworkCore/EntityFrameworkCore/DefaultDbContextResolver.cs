...
public class DefaultDbContextResolver : IDbContextResolver, ITransientDependency
{
    ...
    {
        if (_iocResolver.IsRegistered<IADTOSharpDbContextConfigurer<TDbContext>>())
        {
            var configuration = new ADTOSharpDbContextConfiguration<TDbContext>(connectionString, existingConnection);
            configuration.DbContextOptions.UseApplicationServiceProvider(_iocResolver.Resolve<IServiceProvider>());

            using (var configurer = _iocResolver.ResolveAsDisposable<IADTOSharpDbContextConfigurer<TDbContext>>())
            {
                configurer.Object.Configure(configuration);
            }

            return configuration.DbContextOptions.AddADTOSharpDbContextOptionsExtension().Options;
        }

        if (_iocResolver.IsRegistered<DbContextOptions<TDbContext>>())
        {
            return _iocResolver.Resolve<DbContextOptions<TDbContext>>().WithExtension(new ADTOSharpDbContextOptionsExtension()).As<DbContextOptions<TDbContext>>(); ;
        }

        throw new ADTOSharpException($"Could not resolve DbContextOptions for {typeof(TDbContext).AssemblyQualifiedName}.");
    }
}