...
public class ADTOSharpAutoMapperModule : ADTOSharpModule
{
    private readonly ITypeFinder _typeFinder;

    public ADTOSharpAutoMapperModule(ITypeFinder typeFinder)
    {
    ...
    {
        IocManager.Register<IADTOSharpAutoMapperConfiguration, ADTOSharpAutoMapperConfiguration>();

        Configuration.ReplaceService<ObjectMapping.IObjectMapper, AutoMapperObjectMapper>();

        Configuration.Modules.ADTOSharpAutoMapper().Configurators.Add(CreateCoreMappings);
    }
    ...
    private void CreateMappings()
    {
        Action<IMapperConfigurationExpression> configurer = configuration =>
        {
            ...
            foreach (var configurator in Configuration.Modules.ADTOSharpAutoMapper().Configurators)
            {
...
#pragma warning disable CS0618 // Type or member is obsolete, this line will be removed once AutoMapper is updated
        ADTOSharpEmulateAutoMapper.Mapper = mapper;
