...
public class ADTOSharpRoleManager<TRole, TUser> : RoleManager<TRole>, IDomainService
    ...
    where TUser : ADTOSharpUser<TUser>
{
    ...
    private IRolePermissionStore<TRole> RolePermissionStore
    {
        get
        {
            ...
            {
                throw new ADTOSharpException("Store is not IRolePermissionStore");
            }
    ...
    protected ADTOSharpRoleStore<TRole, TUser> ADTOSharpStore { get; }
    ...
    public ADTOSharpRoleManager(
        ADTOSharpRoleStore<TRole, TUser> store,
        IEnumerable<IRoleValidator<TRole>> roleValidators,
        ILookupNormalizer keyNormalizer,
        IdentityErrorDescriber errors,
        ILogger<ADTOSharpRoleManager<TRole, TUser>> logger,
    ...
    /// <exception cref="ADTOSharpException">Throws exception if no role with given id</exception>
    public virtual async Task<TRole> GetRoleByIdAsync(Guid roleId)
    {
        var role = await FindByIdAsync(roleId.ToString());
        if (role == null)
        {
            throw new ADTOSharpException("There is no role with id: " + roleId);
        }
    ...
    /// <param name="roleName">Role name</param>
    /// <returns>Role</returns>
    /// <exception cref="ADTOSharpException">Throws exception if no role with given roleName</exception>
    public virtual async Task<TRole> GetRoleByNameAsync(string roleName)
    {
        var role = await FindByNameAsync(roleName);
        if (role == null)
        {
            throw new ADTOSharpException("There is no role with name: " + roleName);
        }
    ...
    public virtual TRole GetRoleByName(string roleName)
    {
        ...
        var role = ADTOSharpStore.FindByName(normalizedRoleName);
        if (role == null)
        {
            throw new ADTOSharpException("There is no role with name: " + roleName);
        }
        ...
        return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            if (!includeChildren)
            {
                var query = from organizationUnitRole in await _organizationUnitRoleRepository.GetAllAsync()
                            join role in await ADTOSharpStore.GetRolesAsync() on organizationUnitRole.RoleId equals role.Id
    ...
    {
        return ADTOSharpStore.FindByDisplayNameAsync(displayName);
    }
    ...
    private async Task<RolePermissionCacheItem> GetRolePermissionCacheItemAsync(Guid roleId)
    {
        ...
        return await _cacheManager.GetRolePermissionCache().GetAsync(cacheKey, async () =>
        {
            ...
            if (role == null)
            {
                throw new ADTOSharpException("There is no role with given id: " + roleId);
            }
    ...
    private RolePermissionCacheItem GetRolePermissionCacheItem(Guid roleId)
    {
        ...
        return _cacheManager.GetRolePermissionCache().Get(cacheKey, () =>
        {
            ...
            var role = ADTOSharpStore.FindById(roleId.ToString(), CancellationToken);
            if (role == null)
            {
                throw new ADTOSharpException("There is no role with given id: " + roleId);
            }
