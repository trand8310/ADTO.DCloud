...
public class ADTOSharpUserManager<TRole, TUser> : UserManager<TUser>, IDomainService
    ...
    where TUser : ADTOSharpUser<TUser>
{
    protected IUserPermissionStore<TUser> UserPermissionStore
    {
        get
        {
            ...
            {
                throw new ADTOSharpException("Store is not IUserPermissionStore");
            }
    ...
    protected ADTOSharpUserStore<TRole, TUser> ADTOSharpUserStore { get; }
    ...
    public ADTOSharpUserManager(
        ADTOSharpRoleManager<TRole, TUser> roleManager,
        ADTOSharpUserStore<TRole, TUser> userStore,
    ...
    public virtual Task<IQueryable<TUser>> GetUsersAsync()
        => ADTOSharpUserStore.GetUsersAsync();
    ...
    {
        return ADTOSharpUserStore.FindByNameOrEmailAsync(userNameOrEmailAddress);
    }

    public virtual TUser FindByNameOrEmail(string userNameOrEmailAddress)
    {
        return ADTOSharpUserStore.FindByNameOrEmail(userNameOrEmailAddress);
    }

    public virtual Task<List<TUser>> FindAllAsync(UserLoginInfo login)
    {
        return ADTOSharpUserStore.FindAllAsync(login);
    }

    public virtual List<TUser> FindAll(UserLoginInfo login)
    {
        return ADTOSharpUserStore.FindAll(login);
    }

    public virtual Task<TUser> FindAsync(Guid? tenantId, UserLoginInfo login)
    {
        return ADTOSharpUserStore.FindAsync(tenantId, login);
    }

    public virtual TUser Find(Guid? tenantId, UserLoginInfo login)
    {
        return ADTOSharpUserStore.Find(tenantId, login);
    }

    public virtual Task<TUser> FindByNameOrEmailAsync(Guid? tenantId, string userNameOrEmailAddress)
    {
        return ADTOSharpUserStore.FindByNameOrEmailAsync(tenantId, userNameOrEmailAddress);
    }

    public virtual TUser FindByNameOrEmail(Guid? tenantId, string userNameOrEmailAddress)
    {
        return ADTOSharpUserStore.FindByNameOrEmail(tenantId, userNameOrEmailAddress);
    }
    ...
    /// <exception cref="ADTOSharpException">Throws exception if no user found with given id</exception>
    public virtual async Task<TUser> GetUserByIdAsync(Guid userId)
    {
        var user = await FindByIdAsync(userId.ToString());
        if (user == null)
        {
            throw new ADTOSharpException("There is no user with id: " + userId);
        }
    ...
    {
        var user = ADTOSharpUserStore.FindById(userId.ToString());
        if (user == null)
        {
            throw new ADTOSharpException("There is no user with id: " + userId);
        }
    ...
    public override async Task<IdentityResult> UpdateAsync(TUser user)
    {
        ...
        {
            if ((await GetOldUserNameAsync(user.Id)) == ADTOSharpUserBase.AdminUserName)
            {
                throw new UserFriendlyException(
                    string.Format(L("CanNotRenameAdminUser"), ADTOSharpUserBase.AdminUserName));
            }
        }
    ...
    //    if (user.UserName != ADTOSharpUserBase.AdminUserName)
    //    {
    //        if ((GetOldUserName(user.Id)) == ADTOSharpUserBase.AdminUserName)
    //        {
    //            throw new UserFriendlyException(string.Format(L("CanNotRenameAdminUser"), ADTOSharpUserBase.AdminUserName));
    //        }
    //    }
    ...
    public override async Task<IdentityResult> DeleteAsync(TUser user)
    {
        if (user.UserName == ADTOSharpUserBase.AdminUserName)
        {
            throw new UserFriendlyException(string.Format(L("CanNotDeleteAdminUser"), ADTOSharpUserBase.AdminUserName));
        }
    ...
    public virtual async Task<IdentityResult> ChangePasswordAsync(TUser user, string newPassword)
    {
        ...
        await ADTOSharpUserStore.SetPasswordHashAsync(user, PasswordHasher.HashPassword(user, newPassword));
    ...
    //    ADTOSharpUserStore.SetPasswordHash(user, PasswordHasher.HashPassword(user, newPassword));
    //    return IdentityResult.Success;
    //}

    public virtual async Task<IdentityResult> CheckDuplicateUsernameOrEmailAddressAsync(Guid? expectedUserId,
    ...
    {
        await ADTOSharpUserStore.UserRepository.EnsureCollectionLoadedAsync(user, u => u.Roles);
    ...
    private async Task CheckMaxUserOrganizationUnitMembershipCountAsync(Guid? tenantId, int requestedCount)
    {
        ...
        {
            throw new ADTOSharpException($"Can not set more than {maxCount} organization unit for a user!");
        }
    }
    ...
    public virtual void InitializeOptions(Guid? tenantId)
    {
        ...
        Options.Lockout.MaxFailedAccessAttempts = GetSettingValue<int>(
            ADTOSharpZeroSettingNames.UserManagement.UserLockOut.MaxFailedAccessAttemptsBeforeLockout, tenantId);
    ...
    {
        return ADTOSharpUserStore.GetUserNameFromDatabaseAsync(userId);
    }

    protected virtual string GetOldUserName(Guid userId)
    {
        return ADTOSharpUserStore.GetUserNameFromDatabase(userId);
    }
    ...
    private UserPermissionCacheItem GetUserPermissionCacheItem(Guid userId)
    {
        ...
        return _cacheManager.GetUserPermissionCache().Get(cacheKey, () =>
        {
            ...
            foreach (var roleName in ADTOSharpUserStore.GetRoles(user))
            {
    ...
    {
        await ADTOSharpUserStore.AddTokenValidityKeyAsync(user, tokenValidityKey, expireDate, cancellationToken);
    }
    ...
    public virtual void AddTokenValidityKey(
        ...
        CancellationToken cancellationToken = default(CancellationToken))
    {
        ADTOSharpUserStore.AddTokenValidityKey(user, tokenValidityKey, expireDate, cancellationToken);
    }
    ...
    public virtual async Task<bool> IsTokenValidityKeyValidAsync(
        ...
        CancellationToken cancellationToken = default(CancellationToken))
    {
        return await ADTOSharpUserStore.IsTokenValidityKeyValidAsync(user, tokenValidityKey, cancellationToken);
    }
    ...
    public virtual bool IsTokenValidityKeyValid(
        ...
        CancellationToken cancellationToken = default(CancellationToken))
    {
        return ADTOSharpUserStore.IsTokenValidityKeyValid(user, tokenValidityKey, cancellationToken);
    }
    ...
    public virtual async Task RemoveTokenValidityKeyAsync(
        ...
        CancellationToken cancellationToken = default(CancellationToken))
    {
        await ADTOSharpUserStore.RemoveTokenValidityKeyAsync(user, tokenValidityKey, cancellationToken);
    }
    ...
    public virtual void RemoveTokenValidityKey(
        ...
        CancellationToken cancellationToken = default(CancellationToken))
    {
        ADTOSharpUserStore.RemoveTokenValidityKey(user, tokenValidityKey, cancellationToken);
    }

    public bool IsLockedOut(string userId)
    {
        var user = ADTOSharpUserStore.FindById(userId);
        if (user == null)
        {
            throw new ADTOSharpException("There is no user with id: " + userId);
        }

        var lockoutEndDateUtc = ADTOSharpUserStore.GetLockoutEndDate(user);
    ...
    {
        ADTOSharpUserStore.ResetAccessFailedCount(user);
    }
}