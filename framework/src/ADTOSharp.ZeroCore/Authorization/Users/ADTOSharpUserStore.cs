...
/// </summary>
public class ADTOSharpUserStore<TRole, TUser> :
    ...
    where TUser : ADTOSharpUser<TUser>
{
        ...
        CancellationToken cancellationToken = default(CancellationToken))
    {
        return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            ...
            catch (ADTOSharpDbConcurrencyException ex)
            {
        ...
        CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            return await UserRepository.FirstOrDefaultAsync(userId.To<Guid>());
        });
    }
    ...
    /// <returns>A <see cref="Task{TResult}"/> 它包含用户所属的角色。</returns>
    ...
    /// <param name="cancellationToken">取消操作的通知</param>
    /// <returns>A <see cref="Task{TResult}"/>包含一个BOOL标志，指示指定的用户是否是给定角色的成员。</returns>
        ...
        return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            ...
            var query = from userLogin in await _userLoginRepository.GetAllAsync()
                        ...
                        where userLogin.LoginProvider == loginProvider &&
                                  ...
                                  userLogin.TenantId == ADTOSharpSession.TenantId
                        select user;
        ...
        return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            ...
            var query = from userclaims in await _userClaimRepository.GetAllAsync()
                        ...
                        where userclaims.ClaimValue == claim.Value && userclaims.ClaimType == claim.Type &&
                                  userclaims.TenantId == ADTOSharpSession.TenantId
                        select user;
    ...
    public virtual async Task<TUser> FindAsync(Guid? tenantId, UserLoginInfo login)
    {
        return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
        {
            using (_unitOfWorkManager.Current.SetTenantId(tenantId))
    ...
    public virtual async Task<string> GetUserNameFromDatabaseAsync(Guid userId)
    {
    ...
    public virtual string GetUserNameFromDatabase(Guid userId)
    {
        using (var uow = _unitOfWorkManager.Begin(new UnitOfWorkOptions
        {
            Scope = TransactionScopeOption.Suppress
